    ...
    appendLine(
        """
            @Spread factory method return type must not declare @Nullable spreadable methods:
              ...
              [Method]
                @Nullable ${error.spreadMethod.returnType.simpleName} ${error.spreadClass.qualifiedName}.${error.spreadMethod.name}

              [Factory Method]
                ${error.objects.qualifiedName}.${error.factoryMethod.name}
