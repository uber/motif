...
class ChildDeclarationFactory(override val env: ProcessingEnvironment) : JavaxUtil {
    ...
    private val generatedDependenciesFactory = GeneratedDependenciesFactory(env)
    ...
    fun create(executable: Executable): ChildDeclaration {
        ...
        val method = ChildMethod(executable, childScopeType.ir, dynamicDependencies)
        val generatedDependencies = generatedDependenciesFactory.create(childScopeType)
        return ChildDeclaration(method, generatedDependencies)
    }
}