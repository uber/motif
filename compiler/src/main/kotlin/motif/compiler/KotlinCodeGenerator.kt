...
object KotlinCodeGenerator {
  ...
  private fun ScopeImpl.spec(classNameToUse: com.squareup.kotlinpoet.ClassName): TypeSpec =
      TypeSpec.classBuilder(classNameToUse)
          .apply {
            ...
            factoryProviderMethods.forEach { addFunctions(it.specs(isBaselineStrategy = isBaselineStrategy, perDependencyLockFields = perDependencyLockFields)) }
  ...
  // [Caching strategy] Adds MotifLock fields and config cache when per-dependency locking is enabled.
    ...
    return TypeSpec.classBuilder(className.kt)
        .apply {
          ...
          primaryConstructor(wrapperConstructor)
            ...
            // Delegate all access methods to the delegate
            accessMethodImpls
                .filter { !it.overriddenMethod.isSynthetic }
                .forEach {
                    addFunction(
            ...
            accessMethodImpls
                .filter { it.overriddenMethod.isSynthetic }
                .forEach {
                    val propName = with(it.overriddenMethod.name) {
                        when {
