...
object JavaCodeGenerator {
  ...
  private fun ScopeImplAnnotation.spec(): AnnotationSpec {
    return AnnotationSpec.builder(motif.ScopeImpl::class.java)
        .apply {
          if (children.isEmpty()) {
            addMember("children", "{}")
          } else {
  ...
  private fun DependenciesField.spec(): FieldSpec {
    return FieldSpec.builder(dependenciesClassName.j, name, Modifier.PRIVATE, Modifier.FINAL)
        .build()
  }
  ...
  private fun Dependencies.spec(): TypeSpec {
    return TypeSpec.interfaceBuilder(className.j)
        .apply {
          addModifiers(Modifier.PUBLIC)
