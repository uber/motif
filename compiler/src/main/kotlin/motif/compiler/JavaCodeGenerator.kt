...
object JavaCodeGenerator {
    ...
    val classNameToUse =
        if (variantSuffix != null) {
          com.squareup.javapoet.ClassName.get(
              className.j.packageName(), className.j.simpleName() + variantSuffix)
        } else {
          className.j
        }
  ...
  private fun ScopeImpl.wrapperSpec(): TypeSpec {
    val delegateField =
        FieldSpec.builder(superClassName.j, "delegate", Modifier.PRIVATE, Modifier.FINAL).build()
    ...
    val wrapperConstructor =
        MethodSpec.constructorBuilder()
            ...
            .beginControlFlow(
                "if (\$T.cachingStrategy == \$T.SMART_CACHE)",
                com.squareup.javapoet.ClassName.get("motif", "MotifRuntimeConfig"),
                com.squareup.javapoet.ClassName.get("motif", "CachingStrategy"))
            .addStatement(
                "this.delegate = new \$T(dependencies)",
                com.squareup.javapoet.ClassName.get(
                    className.j.packageName(), className.j.simpleName() + "_SmartCache"))
          ...
          childMethodImpls.forEach { childMethod ->
            addMethod(
                MethodSpec.methodBuilder(childMethod.childMethodName)
                    ...
                    .apply {
                      childMethod.parameters.forEach { param -> addParameter(param.spec()) }
                    }
                    .addStatement(
                        "return delegate.\$N(\$L)",
  ...
  private fun Constructor.spec(perDependencyLockFields: PerDependencyLockFields?): MethodSpec {
    ...
    if (perDependencyLockFields != null && perDependencyLockFields.locks.isNotEmpty()) {
      builder.addStatement(
          "boolean usePerDependencyLocking = \$T.usePerDependencyLock",
          com.squareup.javapoet.ClassName.get("motif", "MotifRuntimeConfig"))
    }
    ...
    perDependencyLockFields?.locks?.values?.forEach { lockFieldName ->
      builder.addStatement(
          "this.\$N = usePerDependencyLocking ? new \$T() : null",
          lockFieldName,
          com.squareup.javapoet.ClassName.get("motif", "MotifLock"))
    }
    ...
    if (!isBaselineStrategy) {
      ...
      if (lockFieldName != null) {
        builder.beginControlFlow(
            "synchronized(\$N != null ? \$N : this)", lockFieldName, lockFieldName)
      } else {
