...
private constructor(
  ...
  private inner class Factory(private val scope: Scope) {
    ...
    fun create(): List<ScopeImpl> {
      ...
      if (cachingStrategy == motif.CachingStrategy.RUNTIME_SELECTABLE) {
        return listOf(
            createWrapper(),
            createVariant(
                motif.CachingStrategy.BASELINE_WITH_LOCK_SELECTABLE,
                "_BaselineSelectableLock",
            ),
    ...
    private fun createWrapper(): ScopeImpl {
      ...
      return ScopeImpl(
          ...
          objectsImpl =
              objectsImpl(), // Wrapper needs Objects nested class for variants to reference
          dependencies =
              dependencies(
                  forceGenerate = true,
              ), // Wrapper needs Dependencies nested class for variants to reference
    ...
    private fun createVariant(
        cachingStrategy: motif.CachingStrategy,
        variantSuffix: String?,
    ): ScopeImpl {
      val isInternal = (scope.clazz as? CompilerClass)?.isInternal() ?: false
      val isBaselineStrategy =
    ...
    private fun perDependencyLockFields(
        cachingStrategy: motif.CachingStrategy,
        useSelectiveCaching: Boolean,
    ): PerDependencyLockFields? {
      ...
      val locks =
          scope.factoryMethods
              .filter { factoryMethod ->
                if (useSelectiveCaching) {
                  !shouldSkipCaching(factoryMethod, useSelectiveCaching) &&
    ...
    private fun childMethodImpls(
        cachingStrategy: motif.CachingStrategy,
        variantSuffix: String?,
    ): List<ChildMethodImpl> =
        graph.getChildEdges(scope).map { childEdge ->
          childMethodImpl(childEdge, cachingStrategy, variantSuffix)
        }
    ...
    private fun childDependenciesImpl(
      ...
      val parentScopeClassName =
          if (variantSuffix != null) {
            ClassName.get(
                scope.implClassName.j.packageName(),
                scope.implClassName.j.simpleName() + variantSuffix,
            )
          } else {
            scope.implClassName
          }
    ...
    private fun shouldCache(factoryMethod: FactoryMethod): Boolean {
      ...
      shouldCacheCache[returnType]?.let {
        return it
      }
    ...
    /**
     * Checks if a method is a simple passthrough (abstract single-parameter method that just
     * casts/forwards without construction overhead).
     */
    ...
    private fun countInternalUsage(type: Type): Int = dependentsCache[type]?.size ?: 0
  }
  ...
  /**
   * Resolves the caching strategy from the @Scope annotation. Defaults to BASELINE for backwards
   * compatibility and safety.
   */
  private fun resolveCachingStrategy(
      scopeAnnotation: motif.ast.IrAnnotation,
  ): motif.CachingStrategy {
    val strategyValue =
        scopeAnnotation.annotationValueMap[SCOPE_ANNOTATION_FIELD_CACHING_STRATEGY]
