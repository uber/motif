...
/**
 * Verifies that BASELINE generates byte-for-byte identical code to pre-change output. BASELINE is
 * the control arm of the caching-strategy A/B test.
 *
...
class BaselineCodegenTest(
  ...
  companion object {
    ...
    fun data(): Collection<Array<Any>> =
        CASES.flatMap { case -> OutputMode.values().map { mode -> arrayOf<Any>(case, mode) } }
  }
